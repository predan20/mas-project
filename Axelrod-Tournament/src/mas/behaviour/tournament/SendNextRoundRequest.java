 ...
 * Sends a message to all players containing the oponent's last action and
 * handles the "inform" message by retrieving the resulting player action.
 */
    ...
    private ACLMessage newMessage(ACLMessage baseMessage, AID receiver, AID oponent) {
        ...
        try {
            myAgent.getContentManager().fillContent(message,
                    new Action(getTournament().getAID(), new NextRound(oponentLastAction)));
        } catch (CodecException e) {
            throw new RuntimeException(e);
        } catch (OntologyException e) {
            throw new RuntimeException(e);
